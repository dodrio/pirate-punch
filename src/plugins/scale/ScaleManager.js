...
import GameEvents from 'phaser/core/events'
import GetInnerHeight from 'phaser/dom/GetInnerHeight'
import GetTarget from 'phaser/dom/GetTarget'
import GetScreenOrientation from 'phaser/dom/GetScreenOrientation'
import NOOP from 'phaser/utils/NOOP'
import Rectangle from 'phaser/geom/rectangle/Rectangle'
import Size from 'phaser/structs/Size'
import SnapFloor from 'phaser/math/snap/SnapFloor'
import Vector2 from 'phaser/math/Vector2'
...
var ScaleManager = new Class({
  ...
  initialize: function ScaleManager(game) {
    ...
    /**
     * The parent object of the game container. Often a div, or the browser window, or nothing in non-browser environments.
     *
    ...
    /**
     * The parent object of the Canvas.
     */
    ...
    /**
     * The Scaled Size component.
     *
     * The size used for position gameContainer. The unit of this size is CSS px.
     *
     * @name Phaser.Scale.ScaleManager#scaledSize
     * @type {Phaser.Structs.Size}
     * @since 3.17.0
     */
    ...
    /**
     * The Viewport Size component.
     *
     * The size used for position gameContainer.
     *
     * @name Phaser.Scale.ScaleManager#viewportSize
     * @type {Phaser.Structs.Size}
     * @since 3.17.0
     */
     ...
     * @since 3.16.0
     */
    this.scaleMode = CONST.SCALE_MODE.FIT
  ...
  boot: function() {
    ...
    // the canvas css size is same as its parent
  ...
  parseConfig: function(config) {
    ...
    if (config.minWidth > 0) {
      this.scaledSize.setMin(config.minWidth * zoom, config.minHeight * zoom)
    }

    if (config.maxWidth > 0) {
      this.scaledSize.setMax(config.maxWidth * zoom, config.maxHeight * zoom)
    }
    ...
    this.scaledSize.setSize(width, height)
  ...
  getParent: function(config) {
    ...
    this.gameContainer = GetTarget(parent)
    this.gameContainer.style.width = this.gameSize.width + 'px'
  ...
  getParentBounds: function() {
    ...
    var isParentResized =
      this.parentOriginalSize.width !== newOriginalWidth ||
      this.parentOriginalSize.height !== newOriginalHeight
   ...
   *
   * This method will set the `gameSize`, `baseSize` and `scaledSize` components to the given
  ...
  resize: function(width, height) {
    ...
    this.scaledSize.setSize(
      width * zoom * resolution,
      height * zoom * resolution
    )
    ...
    if (styleWidth !== width || styleHeight !== height) {
      this.gameContainer.style.width = styleWidth + 'px'
      this.gameContainer.style.height = styleHeight + 'px'
    }
  ...
  updateScale: function() {
    this.scaledSize.setSize(this.parentSize.width, this.parentSize.height)
  ...
  updateDisplayScale: function() {
    this.displayScale.set(
      this.baseSize.width / this.scaledSize.width,
      this.baseSize.height / this.scaledSize.height
    )
  },
  ...
  /**
   * Updates the `viewportSize` size.
   *
   * @method Phaser.Scale.ScaleManager#updateViewport
   * @since 3.17.0
   */
  ...
  getAxisX: function(pageX, pageY) {
    var absoluteX = this.shouldRotate ? pageY : pageX
    var x = (absoluteX - this.canvasBounds.left) * this.displayScale.x
    return x
  },
  ...
  getAxisY: function(pageX, pageY) {
    ...
    if (this.shouldRotate) {
      absoluteY = pageX
      offsetY = this.scaledSize.height
      return (
        (offsetY - (absoluteY - this.canvasBounds.top)) * this.displayScale.y
      )
    } else {
      absoluteY = pageY
      return (absoluteY - this.canvasBounds.top) * this.displayScale.y
    }
  },
